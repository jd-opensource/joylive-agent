    ...
    public SecurityProxy(List<String> serverList, NacosRestTemplate nacosRestTemplate) {
        ...
        if (serverList != null) {
            for (String server : serverList) {
                URI uri = URI.parse(server).path(null).parameters(null).user(null).password(null);
                newServers.add(uri.toString());
            }
        }

        clientAuthPluginManager = new ClientAuthPluginManager();
        clientAuthPluginManager.init(newServers, nacosRestTemplate);
    }
