 ...
 */
package com.jd.live.agent.plugin.router.springcloud.v5.registry;

import com.jd.live.agent.governance.registry.ServiceEndpoint;
import com.jd.live.agent.governance.registry.ServiceRegistry;
import com.jd.live.agent.plugin.router.springcloud.v5.instance.SpringEndpoint;
...
public class SpringServiceRegistry implements ServiceRegistry {
    ...
    private static final String FIELD_SERVICE_INSTANCE_LIST_SINGLETON_SUPPLIER = "serviceInstanceListSingletonSupplier";
    ...
    public CompletableFuture<List<ServiceEndpoint>> getEndpoints() {
        ...
        if (supplier != null) {
            result = supplier.get()
                    ...
                    .collectList()
                    .map(instances -> toList(instances, s -> new SpringEndpoint(service, s)))
                    .toFuture();
        } else {
    ...
    private static ServiceInstanceListSupplier getSupplier(ReactiveLoadBalancer<ServiceInstance> balancer) {
        ...
        // NacosLoadBalancer
        // RoundRobinLoadBalancer
        ObjectProvider<ServiceInstanceListSupplier> objectProvider = getQuietly(balancer, FIELD_SERVICE_INSTANCE_LIST_SUPPLIER_PROVIDER);
        try {
            if (objectProvider != null) {
                return objectProvider.getIfAvailable();
            }
            SingletonSupplier<ServiceInstanceListSupplier> singletonSupplier = getQuietly(balancer, FIELD_SERVICE_INSTANCE_LIST_SINGLETON_SUPPLIER);
            return singletonSupplier == null ? null : singletonSupplier.get();
