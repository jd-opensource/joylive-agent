 ...
 */
public class OkClientClusterResponse extends AbstractHttpOutboundResponse<CloseableHttpResponse> implements SpringClusterResponse {

    private static final Logger logger = LoggerFactory.getLogger(OkClientClusterResponse.class);

    private byte[] body;

    public OkClientClusterResponse(CloseableHttpResponse response) {
        super(response);
    }

    public OkClientClusterResponse(ServiceError error, ErrorPredicate predicate) {
    ...
    public static CloseableHttpResponse create(HttpRequest request, DegradeConfig degradeConfig) {
        LiveCloseableHttpResponse response = new LiveCloseableHttpResponse(
                HttpVersion.HTTP_1_1,
                degradeConfig.getResponseCode(),
                "OK"
        );
    ...
    private static class LiveCloseableHttpResponse extends BasicHttpResponse implements CloseableHttpResponse {

        LiveCloseableHttpResponse(StatusLine statusline, ReasonPhraseCatalog catalog, Locale locale) {
            super(statusline, catalog, locale);
        }

        LiveCloseableHttpResponse(StatusLine statusline) {
            super(statusline);
        }

        LiveCloseableHttpResponse(ProtocolVersion ver, int code, String reason) {
