    ...
    public FeignException createException(R request, HttpStatus status, String message, Throwable throwable) {
        ...
        switch (status) {
            case SERVICE_UNAVAILABLE:
                return new FeignException.ServiceUnavailable(message, req, req.body());
            case FORBIDDEN:
                return new FeignException.Forbidden(message, req, req.body());
            default:
                return new FeignException.InternalServerError(message, req, req.body());
        }
    }
}
