...
import com.jd.live.agent.plugin.protection.jdbc.connection.LivePoolConnection;
 ...
 */
public abstract class AbstractCreateConnectionInterceptor<T extends LivePoolConnection> extends AbstractDbConnectionInterceptor<T> {
    ...
    public void onSuccess(ExecutableContext ctx) {
        ...
        LiveDriverConnection driver = unwrap(connection);
        if (driver != null) {
     ...
     */
    protected abstract T build(Connection connection, ClusterRedirect address, LiveDriverConnection driver, Object target);
     ...
     */
    protected LiveDriverConnection unwrap(Connection connection) {
        try {
            return connection.unwrap(LiveDriverConnection.class);
