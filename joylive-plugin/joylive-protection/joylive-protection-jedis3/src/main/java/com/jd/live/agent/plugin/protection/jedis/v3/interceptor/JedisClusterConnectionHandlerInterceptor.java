...
public class JedisClusterConnectionHandlerInterceptor extends InterceptorAdaptor {
    ...
    public void onEnter(ExecutableContext ctx) {
        ...
        int index = ctx.getArgumentCount() - 1;
        JedisClientConfig config = ctx.getArgument(index);
        ctx.setArgument(index, new JedisConfig(config, toList(clusterNodes, HostAndPort::toString).toArray(new String[0])));
    }
}
