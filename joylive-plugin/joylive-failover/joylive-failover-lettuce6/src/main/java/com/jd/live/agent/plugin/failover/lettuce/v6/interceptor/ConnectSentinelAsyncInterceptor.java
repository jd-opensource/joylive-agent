...
/**
 * ConnectSentinelAsyncInterceptor
 */
public class ConnectSentinelAsyncInterceptor extends AbstractLettuceInterceptor {

    private static final Logger logger = LoggerFactory.getLogger(ConnectSentinelAsyncInterceptor.class);

    public ConnectSentinelAsyncInterceptor(InvocationContext context) {
        super(context, PRIMARY_ADDRESS_RESOLVER);
    }

    @Override
    public void onEnter(ExecutableContext ctx) {
        // for lookup redis method or recreate redis connection
    ...
    public void onSuccess(ExecutableContext ctx) {
        ...
        ConnectionFuture<StatefulRedisSentinelConnection<?, ?>> future = ((MethodContext) ctx).getResult();
        Function<RedisURI, CompletionStage<?>> recreator = u -> connect(client, u, codec, timeout);
        future.thenAccept(connection -> {
            checkFailover(createConnection(() -> new LettuceSentinelRedisConnection(connection, uri, toClusterRedirect(candidate), closer, recreator)), addressResolver);
        });
    }
    ...
    private static class Accessor {
        ...
        private static final Method connect = ClassUtils.getDeclaredMethod(RedisClient.class,
                "connectSentinelAsync", new Class[]{RedisCodec.class, RedisURI.class, Duration.class});
