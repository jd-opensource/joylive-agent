 ...
 */
package com.jd.live.agent.core.bootstrap.resource.tongweb;
...
/**
 * A class that implements the ResourceFinder interface by searching for resources in a tong web application.
 */
@Extension(value = "TongwebBootResourceLoader", order = BootResourceLoader.ORDER_TOMCAT)
public class TongwebBootResourceLoader implements BootResourceLoader {
    ...
    public InputStream getResource(BootResource resource) throws IOException {
        ...
        File webappDirectory = new File(workingDirectory, "autodeploy/");
        if (!webappDirectory.exists()) {
            String home = System.getProperty("tongweb.home");
            if (home != null) {
                webappDirectory = new File(home, "autodeploy/");
            }
        }
        if (webappDirectory.exists()) {
