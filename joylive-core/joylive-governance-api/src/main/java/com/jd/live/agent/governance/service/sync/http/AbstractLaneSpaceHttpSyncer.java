...
public abstract class AbstractLaneSpaceHttpSyncer extends AbstractLaneSpaceSyncer<HttpLaneSpaceKey> {
    ...
    @Override
    protected SyncResponse<List<ApiSpace>> parseSpaceList(HttpLaneSpaceKey key, String config) {
    ...
    @Override
    protected SyncResponse<LaneSpace> parseSpace(HttpLaneSpaceKey key, String config) {
        if (config == null || config.isEmpty()) {
            return new SyncResponse<>(SyncStatus.NOT_FOUND, null);
        }
     ...
     */
    private <T> void saveConfig(ApiResponse<LaneSpace> response, ObjectParser parser, String name) {
        // save config to local file
        if (response.getError() == null) {
            saveConfig(response.getResult(), parser, AgentPath.DIR_POLICY_LANE, name);
        }
    }
