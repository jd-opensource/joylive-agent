...
import com.jd.live.agent.governance.mcp.spec.JsonRpcMessage;
import com.jd.live.agent.governance.mcp.spec.JsonRpcRequest;
import com.jd.live.agent.governance.mcp.spec.JsonRpcResponse;
import com.jd.live.agent.governance.mcp.spec.ListResourcesResult;

@Extension(JsonRpcMessage.METHOD_RESOURCES_LIST)
public class ListResourcesHandler implements McpHandler {

    @Override
    public JsonRpcResponse handle(JsonRpcRequest request, McpRequestContext ctx) throws Exception {
        return JsonRpcResponse.createSuccessResponse(request.getId(), new ListResourcesResult());
    }
}
