...
import com.jd.live.agent.governance.mcp.spec.SetLevelRequest;

import java.util.Collections;

@Extension(JsonRpcMessage.METHOD_LOGGING_SET_LEVEL)
public class SetLevelHandler implements McpHandler {

    @Override
    public JsonRpcResponse handle(JsonRpcRequest request, McpRequestContext ctx) throws Exception {
        SetLevelRequest req = ctx.convert(request.getParams(), SetLevelRequest.class);
