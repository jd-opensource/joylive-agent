...
import com.jd.live.agent.core.mcp.spec.v1.InitializedNotification;
...
@Extension(JsonRpcMessage.METHOD_NOTIFICATIONS_INITIALIZED)
public class NotificationInitializedHandler implements McpHandler {

    @Override
    public JsonRpcResponse handle(JsonRpcRequest request, McpRequestContext ctx) throws McpException {
        ctx.getSession().inform(new InitializedNotification());
        return null;
    }
}
